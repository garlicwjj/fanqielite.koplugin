...
开发分支现已显式使用 KOReader 自带 CA 集验证证书链，并在握手后校验证书 SAN。任何链验证失败、证书缺失、SAN 缺失、扩展解析失败或主机名不匹配都会关闭连接并安全失败。该实现只有无凭证公开 GET；不能据此宣称未来的扫码 Cookie 客户端已经安全。
...
`fanqielite/http.lua` 只接受精确的 `https://fanqienovel.com/` 前缀，并通过 LuaSec 自定义连接器设置：
...
- CRL、OCSP 或其他实时撤销检查；
- 未来扫码请求的 Cookie、请求头、日志、重定向和退出行为。
