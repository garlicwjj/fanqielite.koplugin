...
公开传输层不会把 LuaSocket/LuaSec 的原始错误字符串返回 UI。只有响应过大、超时和证书相关错误按固定分类提示，其他连接失败统一给出 Wi-Fi/系统时间检查建议；非字符串异常、状态码和响应头不会被 `tostring()`。网络子进程只接受字符串错误，主界面只显示由插件内部令牌标记的预期提示，其他异常正文统一隐藏。

KOReader v2026.03 的 `socketutil` 会同时改写模块级超时和 LuaSocket/LuaSec 的全局 `TIMEOUT`。公开请求和未启用的敏感请求原型因此也对 `set_timeout()` 与 `reset_timeout()` 自身设置异常边界：配置失败不联网并尽力复位，复位失败丢弃本次响应且要求重启 KOReader。测试只能证明插件不会继续处理响应或泄露异常，不能证明已经抛错的底层框架确实恢复，所以不会把该路径描述为可原地重试。

合成测试验证了正确 SAN、错误 SAN、缺少 SAN、通配符错误匹配、缺少证书和扩展解析异常，也锁定 `verify`、CA 路径、旧协议禁用选项和公开错误 canary 边界。测试不会连接番茄官网，不包含账号或 Cookie。
