...
local function next_codepoint(text, index)
    ...
    local valid_three = continuation(b3) and (
        (b1 == 0xE0 and b2 and b2 >= 0xA0 and b2 <= 0xBF)
        or (b1 >= 0xE1 and b1 <= 0xEC and continuation(b2))
        or (b1 == 0xED and b2 and b2 >= 0x80 and b2 <= 0x9F)
        or (b1 >= 0xEE and b1 <= 0xEF and continuation(b2)))
    if valid_three then
        return (b1 - 0xE0) * 0x1000 + (b2 - 0x80) * 0x40 + b3 - 0x80, index + 3
    end
    local b4 = text:byte(index + 3)
    local valid_four = continuation(b3) and continuation(b4) and (
        (b1 == 0xF0 and b2 and b2 >= 0x90 and b2 <= 0xBF)
        or (b1 >= 0xF1 and b1 <= 0xF3 and continuation(b2))
        or (b1 == 0xF4 and b2 and b2 >= 0x80 and b2 <= 0x8F))
    if valid_four then
...
function Pua.decode(text)
    ...
    local output = {}
    local stats = { pua = 0, unknown = 0, invalid = 0 }
    local index = 1
    while index <= #text do
