...
function NetworkTask.get(url, accept, label)
    ...
    local completed, result = Trapper:dismissableRunInSubprocess(function()
        local called, body, request_err, error_kind = pcall(Http.get, url, accept)
    ...
    if not completed then
        return nil, "操作已取消；本地书架、阅读进度和缓存没有改变。", "cancelled"
    end
    ...
    if not result.ok then
        ...
        return nil, result.error, result.retryable == true and "retryable" or nil
    end
