...
grep -Fq '空书架会依次显示' "$plugin_root/INSTALL.md"
grep -Fq '已有书籍时不会继续保证三个入口相邻' "$plugin_root/INSTALL.md"
grep -Fq '查找本地书架（不联网）' "$plugin_root/INSTALL.md"
grep -Fq '不会访问网络' "$plugin_root/INSTALL.md"
grep -Fq '[未开始 · 共 N 章]' "$plugin_root/INSTALL.md"
...
grep -Fq '不会让普通 EPUB、PDF 或其他插件书籍跳转' "$plugin_root/INSTALL.md"
...
grep -Fq '更长的书先按每 100 章分段' "$plugin_root/README.md"
grep -Fq '长书目录为什么先显示章段' "$plugin_root/INSTALL.md"
...
grep -Fq '不会用占位书名生成残缺备份' "$plugin_root/docs/bookshelf-format.md"
grep -Fq '三个公开进度比例字段尚未证明章节内语义和量纲' "$plugin_root/docs/bookshelf-format.md"
